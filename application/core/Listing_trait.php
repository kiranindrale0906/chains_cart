...
trait Listing_trait  {
  ...
  public function initalized($param,$getData) 
  {
    ...
    $this->count_column = @$param['count_column'];
  ...
  private function mysqli_functions(){
    return array('ROU','MAX','MIN','COU','FOR');
  }
        ...
        foreach ($this->theadColumn as $column) {
          if(isset($column[11]) AND $column[11] == true){
            if(isset($column[6]) && !empty($column[6])){
              ...
              $explode[0] = str_replace(array('SUM(','sum('),'',$explode[0]);
              if(isset($explode[1])){
                $last_character = substr($explode[0], -1);
                $find_round = substr($explode[0], 0, 3);
                if($last_character == ')' && !in_array($find_round,$this->mysqli_functions())){
                  $find_is_bracket = substr($explode[0], 0, 1);
                  if($find_is_bracket == '(')
                    $this->db_conn->select('SUM'.$explode[0].' as '.$explode[1]);
                  else
                    $this->db_conn->select('SUM('.$explode[0].' as '.$explode[1]);
                }else{
                  $this->db_conn->select('SUM('.$explode[0].') as '.$explode[1]);
                ...
                $this->db_conn->select_sum($column[1]);
            }
            else $this->db_conn->select_sum($column[1]);
            $get_sum++;
          }
        }
      endif;
...
/* */
  public function get_filter_column_value($third_column){
    if(isset($this->theadColumn)):
        foreach ($this->theadColumn as $column) {
          if(isset($column[6])){
            $as_exist = explode(' as ',$column[6]);
            $coulmn_set = $as_exist[0];
            if(isset($as_exist[1]) && $as_exist[1] == $third_column)
              return $coulmn_set;
            if(isset($as_exist[0]) && $as_exist[0] == $third_column)
              return $as_exist[0];
          }
          else
            if($column[3] == $third_column)
              return $column[3];
        }
    endif;
  }
  ...
  public function fetch_records($export = false,$count=false,$sum=false) //record will fetch from here..
  {
    ...
    if($count == false):
      ...
      $group  = !empty($this->group_by)?$this->group_by:'';
    else:
      $order  = "";
      $start  =  "";
      $group  = !empty($this->group_by)?$this->group_by:'';
      ...
      $where = $this->filterQuery();
      if($sum == false)
       $this->db_conn->select("COUNT(*) as total_count");
    endif;
  ...
  private function filterQuery()//filter query will execute query according to filter
  {
    ...
    foreach ($where_array as $array_key => $array_value) {
      if(!empty($array_value)){
        $get_column = $this->get_filter_column_value(remove_operators($array_key));
        $get_operator = find_operator($array_key);
        $where_data[$get_column.$get_operator] = $array_value; 
      }
    }
        ...
        foreach ($this->theadColumn as $column) {
          if ($column[1] != 'action' && $column[1] != 'checkbox') {
            ...
            elseif(isset($column[9]) AND $column[9] == 'image' AND $column[9] !== 1)
              ...
              $this->db_conn->select("(CASE 
                              WHEN ".$column[1]." IS NULL 
                              THEN '".$column[10]."'            
                              ELSE ".$column[1]."
                          END) as ".$column[1]);
            else $this->db_conn->select($column[1] . ' as "' .$column[1].'"');
          }
        }
      endif;
      if(!isset($_GET['export']) && empty($_GET['export'])){
        if (!empty($this->extra_select_column)) $this->db_conn->select($this->extra_select_column);
      }
