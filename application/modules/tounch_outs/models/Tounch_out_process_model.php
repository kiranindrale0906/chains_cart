<?php 
class Tounch_out_process_model extends BaseModel{
	public $router_class = 'tounch_out_processes';
    ...
    $rules= array(array('field' => 'tounch_out_processes[in_weight]', 'label' => 'Weight',
  ...
  public function before_validate(){
    if (isset($this->formdata['process_out_wastage_details']) 
      ...
      $process = $this->process_model->find('sum(balance_tounch_out) as in_weight, 100 as out_purity, sum(wastage_lot_purity * balance_tounch_out) / sum(balance_tounch_out) as out_lot_purity',
  ...
  public function save($after_save = true) {
    $srno= $this->process_model->find('count(*) + 1 as srno',array('product_name'=>'Tounch Out','department_name'=>'Melting'))['srno'];
    $start_process=array(
      'lot_no'=>strtoupper('TO-'.sprintf("%02d", $srno)),
    ...
    $process_obj=new tounch_out_melting_process_model($start_process);
    ...
    if (isset($this->formdata['process_out_wastage_details'])) {
      foreach ($this->formdata['process_out_wastage_details'] as $index => $process_out_wastage_detail) {
        if (isset($process_out_wastage_detail['process_id']) && !empty($process_out_wastage_detail['process_id'])) {
          ...
          $process_out_wastage_detail_obj->attributes['out_weight'] = $process['balance_tounch_out'];
          $process_out_wastage_detail_obj->attributes['field_name'] = 'Tounch Out';
