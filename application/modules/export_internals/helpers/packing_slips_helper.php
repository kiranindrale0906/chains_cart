...
function getTableSettings() {
  ...
  if($show=='yes') $where='';
  else $where='packing_slips.status=0';
...
function list_settings() {
  return array(
    array("Packing Slip No", "id", FALSE, "id", TRUE, TRUE),
    ...
    array("Gross Weight", "gross_weight", FALSE, "gross_weight", FALSE, FALSE),
...
function get_row_actions($row, $url, $select_url, $filter) {
  $actions = array();
  $controller = 'export_internals/packing_slips';
  ...
  $actions["Hide"] = array('request' => "http",
                           'url' => ADMIN_PATH.'export_internals/packing_slips/update/'.$row['id'].'?from=view',
                           'confirm_message' => "",
                           'class' => 'red',
                           'data_title' =>'View',
