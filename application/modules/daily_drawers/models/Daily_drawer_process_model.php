<?php 
class Daily_drawer_process_model extends BaseModel{
	public $router_class = 'daily_drawer_processes';
    ...
    $rules= array(array('field' => 'daily_drawer_processes[in_purity]', 'label' => 'Melting',
									      'rules' => 'trim|required|numeric|greater_than[0]',
									      'errors'=>array('required'=>'Please Select Melting')),
                  array('field' => 'daily_drawer_processes[in_weight]', 'label' => 'Weight',
                        'rules' => 'trim|required|numeric|greater_than[0]'
                        ));
    return $rules;
  }
  ...
  public function before_validate(){
    ...
    if (isset($this->formdata['process_out_wastage_details']) 
      ...
      $process = $this->process_model->find('sum(balance_daily_drawer_wastage) as in_weight,
                                             sum(out_purity * balance_daily_drawer_wastage) / sum(balance_daily_drawer_wastage) as out_purity,
                                             sum(out_lot_purity * balance_daily_drawer_wastage * out_purity) / sum(balance_daily_drawer_wastage * out_purity) as out_lot_purity,description',
    ...
    if (isset($this->formdata['process_out_wastage_details'])) {
      foreach ($this->formdata['process_out_wastage_details'] as $index => $process_out_wastage_detail) {
        if (isset($process_out_wastage_detail['process_id']) && !empty($process_out_wastage_detail['process_id'])) {
          ...
          $process_out_wastage_detail_obj->attributes['out_weight'] = $process['balance_daily_drawer_wastage'];
          $process_out_wastage_detail_obj->attributes['field_name'] = 'Daily Drawer Wastage';
