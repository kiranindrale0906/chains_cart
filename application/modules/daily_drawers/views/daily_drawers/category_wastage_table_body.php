...
$w = 'W3';
 foreach ($other_daily_drawer_wastages as $index => $wastage) {
        ...
        <?php
          if (!empty($wastage['hook_kdm_purity'])) {
            load_buttons('anchor',array('name'=>'View',
                                        ...
                                        'href'=>ADMIN_PATH."issue_and_receipts/daily_drawer_wastage_ledgers/create?hook_kdm_purity=".$wastage['hook_kdm_purity']."& not_hook=1"));
          }
        ?>
      </td>
	  	...
	  	<td>Wastage</td>
	  	<td class="text-right"><a href=<?=$href.'column=in_weight&&category=3&&purity='.$wastage['hook_kdm_purity']?>><?=isset($wastage['in_weight'])?four_decimal($wastage['in_weight']):0;?></a></td>

	   <td class="text-right"><a href=<?=$href.'column=out_weight&&category=3&&purity='.$wastage['hook_kdm_purity']?> ><?=isset($wastage['out_weight'])?four_decimal($wastage['out_weight']):0;?></a></td></td>

     	<td class="text-right"><a href=<?=$href.'column=issue_weight&&category=3&&purity='.$wastage['hook_kdm_purity']?> ><?=isset($wastage['issue_weight'])?four_decimal($wastage['issue_weight']):0;?></a></td></td>

	  	<td class="text-right"><a href=<?=$href.'column=balance&&category=3&&purity='.$wastage['hook_kdm_purity']?>><?=isset($wastage['balance'])?four_decimal($wastage['balance']):0;?></a></td>
