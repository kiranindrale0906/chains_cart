         ...
         foreach ($purity_wise_datas as $type_index => $type_data) { 
          ...
          <tr>
              ...
              <td><?php load_buttons('anchor',array('name'=>'Print',
                                      ...
                                      '&type='.str_replace(' ', '_', $type_index)))?></td>
              <td ><?=$type_index ?></td>
              <td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=in_weight&&purity='.$purity_index.'&&chain_name='.str_replace(' ','_',$chain_name)?> ><?=isset($type_data['in'])?$type_data['in']:0 ?></a></td>
              <td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=out_weight&&purity='.$purity_index.'&&chain_name='.str_replace(' ','_',$chain_name)?>><?=isset($type_data['out'])?$type_data['out']:0?></a></td>
              <td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=balance&&purity='.$purity_index.'&&chain_name='.str_replace(' ','_',$chain_name)?> ><?=isset($balance)?four_decimal($balance):0?></a></td>
