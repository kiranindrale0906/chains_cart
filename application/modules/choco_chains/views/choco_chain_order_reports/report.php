...
<h6 class="heading blue bold text-uppercase mb-0">Choco chain order details</h6>
<?php } else { ?>
<h5 class="heading blue bold text-uppercase">Choco chain order report</h5>
<?php } ?>
<?php 
if($orders){
    ...
    <div class="row">
      <div class="col-sm-12">
        <h6 class="heading">Order ID: ORD-<?php echo $order['id']; ?></h6>
          <p class="m0"> TYPE : <?php echo $order['type']; ?> </p>
          <p class="m0"> CHAIN : <?php echo $order['chain']; ?> </p>
          <p class="m0"> MELTING : <?php echo $order['melting']; ?> </p>
          <p class="m0"> TONE : <?php echo $order['tone']; ?> </p>
          <p class="m0"> STATUS : <?php echo $order['status']; ?>
            <?php if($order['status'] == 'OPEN') { ?>
              <a class="btn btn-sm btn-primary ajax_post" href="<?php echo ADMIN_PATH; ?>/choco_chains/choco_chain_orders/update/<?php echo $order['id']; ?>" data-ajax='<?php echo json_encode(array('choco_chain_orders[id]' => $order['id'], 'choco_chain_orders[status]' => 'CLOSED', 'submittype' => 'json')) ?>'>Close</a>
            <?php } ?>
          </p>
      </div>
      <div class="col-sm-12">
