	  		...
	  		// pd($purity_wise_datas);
				 foreach ($purity_wise_datas as $type_index => $type_data) { 
					...
					if($type_index=='GPC Powder'){
					 ...
					 // $balance_fine=(($balance)*$purity_index/100);
					}else{
					$sum_out_weight+=($type_data['out']);
					 $balance=four_decimal($type_data['in']-$type_data['out']);
					 // $balance_fine=four_decimal(($balance)*$purity_index/100);
					}
	  							...
	  							if ($count==0) {
	  								if (!empty($worker)) {
	  									...
	  									load_buttons('anchor',array('name'=>'View',
					                                      	...
					                                      	'href'=>ADMIN_PATH."issue_and_receipts/worker_ledgers/create?worker=".$worker."&&hook_kdm_purity=".$purity_index.$type));
	  								}
	  							}
	  						?>
  						</td>
	  						...
	  						<?php
  								if (!empty($worker)) {
  									load_buttons('anchor',array('name'=>'View',
				                                      	'layout' => 'application',
				                                      	'class'=>'btn-xs bold blue float-left bar_code_genrate',
				                                      	'href'=>ADMIN_PATH."issue_and_receipts/worker_ledgers/create?worker=".$worker."&&hook_kdm_purity=".$purity_index."&&type=".$type_index));
  								}
	  						?>
  						</td>
							<td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=in_weight&&purity='.$purity_index.'&&worker='.str_replace(' ', '_', $worker)?> ><?=isset($type_data['in'])?$type_data['in']:0 ?></a></td>

							<?php if($type_index=='GPC Powder'){?>
							<td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=out_weight&&purity='.$purity_index.'&&worker='.str_replace(' ', '_', $worker)?> ><?=isset($type_data['gpc_powder_out'])?four_decimal($type_data['gpc_powder_out']):0?>
							</a></td>
							<?php }else{?>
							<td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=out_weight&&purity='.$purity_index.'&&worker='.str_replace(' ', '_', $worker)?> ><?=isset($type_data['out'])?four_decimal($type_data['out']):0?>
							</a></td>

							<?php }?>

							<td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=balance&&purity='.$purity_index.'&&worker='.str_replace(' ', '_', $worker)?> ><?=isset($balance)?four_decimal($balance):0?></a></td>
