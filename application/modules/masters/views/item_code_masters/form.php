...
<form method="post" class="form-horizontal fields-group-sm form_radius_none" enctype="multipart/form-data"      action="<?= get_form_action($controller, $action, $record) ?>">
  <div class="row">
    ...
    <?php load_field('dropdown', array('field' => 'design_name','option'=> $design_names)) ?>        
    <?php load_field('text', array('field' => 'item_code')) ?>
    <?php load_field('dropdown', array('field' => 'melting','option'=>$meltings)) ?>
    <?php 
        if(HOST == "ARF"){
          load_field('dropdown', array('field' => 'melting_lot_category_one','option'=> $category_ones)); 
          load_field('dropdown', array('field' => 'machine_size','option'=> $machine_sizes));
        }
    load_field('dropdown', array('field' => 'tone','option'=> $tones));
     ?>
