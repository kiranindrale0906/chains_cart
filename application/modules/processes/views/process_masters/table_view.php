      ...
      <?php
        ...
        if(!empty($record['process_data'])){
          ...
          foreach ($record['process_data'] as $index => $process_data) {
            ...
            ?>
            <?php if(!empty($process_data['balance_gross'])&& ($process_data['product_name']!='Stone Transfer')) { ?>
              <tr style="<?php echo $colour;?>" class="<?php echo str_replace(" ", "_", strtolower($process_data['product_name'])).'_list';?>">
                <td class><a href="javascript:void(1);" 
                      data-collapse="0" class ="show_more_details " 
                      data-chain="<?php echo $process_data['product_name']; ?>" 
                      data-list="<?php echo str_replace(" ", "_", strtolower($process_data['product_name']));?>" 
                      data-hide ="<?php echo str_replace(" ", "_", strtolower($process_data['product_name'])).'_list';?>"
                      data-request="0">
                      <span class="fa fa-plus "></a> 
                </span> <b><?= isset($process_data['product_name'])?$process_data['product_name']:'-'; ?></b></td>
                <td><?= isset($process_data['process_name'])?$process_data['process_name']:'-'; ?></td>
                <td><?= isset($process_data['department_name'])?$process_data['department_name']:'-'; ?></td>
