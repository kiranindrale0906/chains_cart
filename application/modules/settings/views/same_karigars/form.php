<?php echo form_open_multipart(get_form_action($controller,$action, $record), 'method="post" class="form-horizontal fields-group-sm form_radius_none"'); ?>
  <div class="row">
    ...
    load_field('text', array('field' => 'karigar_name'));
    }else{
    load_field('dropdown', array('field' => 'karigar_name', 'option' => $karigar_name));

    }
      } ?>

    <?php 
          load_field('duration', array('field' => 'due_duration','class'=>'durationpicker'));
          //load_field('text', array('field' => 'capacity'));
