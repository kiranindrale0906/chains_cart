...
class Fancy_chain_ledgers extends Ledgers {
  ...
  public function index() {
    redirect(base_url().'issue_and_receipts/fancy_chain_ledgers/create');
  }
  ...
  public function _get_form_data() {
    ...
    $in_fields = 'processes.lot_no as lot_no,
                  ...
                  melting_lot_details.in_purity as purity, 
                  melting_lot_details.required_weight * melting_lot_details.in_purity / 100 as fine, 
    ...
    $bounch_out_fields = 'processes.lot_no as lot_no,
                       ...
                       date(process_details.created_at) as created_date, process_details.created_at as created_at';    
    $in_where           = "melting_lot_details.required_weight != 0 AND (melting_lots.process_name = 'Fancy Chain' OR melting_lots.process_name = 'Fancy 75 Chain')";
    $hook_in_where      = "process_details.daily_drawer_in_weight != 0 AND process_details.karigar = 'Factory Fancy'";
    if(!empty($this->data['record']['in_lot_purity'])) {
      if ($this->data['record']['in_lot_purity'] == '83.65') {
        $in_where      .= ' and (melting_lot_details.in_purity >= 80 and melting_lot_details.in_purity < 85)' ;
        ...
        $in_where.=' and (melting_lot_details.in_purity >= 85 and melting_lot_details.in_purity < 88)' ;
        ...
        $in_where.=' and melting_lot_details.in_purity < 80' ;
        $hook_in_where .= ' and processes.hook_kdm_purity < 80' ;
      } elseif ($this->data['record']['in_lot_purity'] == '100') {
        $in_where.=' and melting_lot_details.in_purity = 100' ;
        $hook_in_where .= ' and processes.hook_kdm_purity = 100' ;
      } else {
        $in_where      .= ' and (melting_lot_details.in_purity >= 88 and melting_lot_details.in_purity < 100)' ;
    ...
    $query = $this->db->query("(select ".$in_fields." from melting_lot_details right join processes on    
                                melting_lot_details.process_id=processes.id right join melting_lots on melting_lot_details.melting_lot_id = melting_lots.id where ".$in_where." order by melting_lot_details.created_at asc)
                              UNION 
                               (select ".$hook_in_fields." from process_details 
                               inner join processes on process_details.process_id = processes.id
                               where ".$hook_in_where." and process_details.daily_drawer_in_weight != 0
                               order by created_at asc)
                              ");
    ...
    $wastage_where      = "process_details.daily_drawer_wastage != 0 AND (processes.product_name = 'Fancy Chain' or processes.product_name = 'Fancy 75 Chain') AND processes.department_name in ('Chain Making','Fancy Hold')";
    $loss_where         = "processes.loss != 0 AND processes.department_name in ('Chain Making','Fancy Hold')                AND (processes.product_name = 'Fancy Chain' or processes.product_name = 'Fancy 75 Chain')";
    $recutting_out_where         = "process_details.recutting_out != 0 AND processes.department_name in ('Chain Making','Fancy Hold')";
    $bounch_out_where         = "process_details.bounch_out != 0 AND (processes.product_name = 'Fancy Chain' or processes.product_name = 'Fancy 75 Chain') AND processes.department_name in ('Fancy Hold')";
