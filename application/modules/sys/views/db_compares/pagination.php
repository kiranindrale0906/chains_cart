<?php 
  ...
  $current_url = base_url().'sys/db_compares/index/'.$table.'?'.$date ;
      ...
      <ul class="pagination pagination_blue pagination-sm m-0">
        <?php if(isset($_GET['page']) && $_GET['page'] !=1){?>
          <li class="page-item previous" aria-controls="datatable" tabindex="0" id="datatable_previous">
            <a class="page-link" data-controller = "<?php echo $this->router->class;?>" href="<?php echo $current_url;?>&page=<?php echo $prev_page_id?>">Previous</a>
          </li>
        <?php }?>
            ...
            if($i <= $total_pages){
              ...
              ?>
                <li class="page-item <?php echo $class; ?>" aria-controls="datatable" tabindex="0">
                  <a class="page-link" data-controller = "<?php echo $this->router->class;?>" href="<?php echo $current_url?>&page=<?=$i;?>">
                    <?=$i;?>
                  </a>
                </li>
              <?php 
        ...
        <?php if($pages != $page_no && $pages !=0){?>
          <li class="page-item next" aria-controls="datatable" tabindex="0" id="datatable_next">
            <a class="page-link " data-controller = "<?php echo $this->router->class;?>" href="<?php echo $current_url?>&page=<?php echo $next_page_id;?>">Next</a>
          </li>
        <?php }?>
      </ul>
    </div>
