...
<form method="post" class="form-horizontal fields-group-sm form_radius_none" 
      ...
      action=<?= get_form_action('qr_codes/generate_lot_taggings','store?print='.$_GET['print'], @$record) ?>>
...
<table class="table table-sm table_blue" id="tblAddRow">
  ...
  <tbody>
    <?php
    if(!empty($tagging_details)){
        $sum_weight=$sum_quantity=0;
        foreach ($tagging_details as $tagging_detail_index => $value) {
        //$sum_weight+=$value['lot_weight'];
        //$sum_quantity+=$value['lot_qty'];?>
        <tr class="order_<?= $value['id']?>">
          <td>
            <?php load_field('checkbox', array('field' => 'generate_lot_tagging_detail_id',
                                               'class' => 'qr_code_detail_id',
                                               'index' => $tagging_detail_index,
                                               'option' => array(
                                                            array('chk_id' => $tagging_detail_index,
                                                                  'value' => $value['lot_no'],
                                                                  'label' => '',
                                                                  'checked' => (!empty($generate_lotes_out_wastage_details[$tagging_detail_index]['id']) 
                                                                                ? 'checked' : ''))),
