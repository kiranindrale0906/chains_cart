...
<tr>
  ...
  <th class="text-right">Box Weight</th>
...
<tbody>
<?php
 $sum_process_in_weight=$sum_process_out_weight=$sum_melting_wastage=$sum_daily_drawer_wastage=$sum_loss=$sum_in_weight=$sum_out_weight=$sum_balance=$sum_balance_fine=$sum_box_weight=$balance=$balance_fine=0;
	  	...
	  	<?php
	  		$count=0;
				 foreach ($purity_wise_datas as $type_index => $type_data) { 
					...
					$sum_process_in_weight+=($type_data['in_weight']);
					$sum_process_out_weight+=($type_data['out_weight']);
					$sum_melting_wastage+=($type_data['melting_wastage']);
					$sum_daily_drawer_wastage+=($type_data['daily_drawer_wastage']);
					$sum_loss+=($type_data['loss']);
					...
					<tr>
	  					...
	  					<td>
	  							...
	  							if ($count==0) {
	  								if (!empty($karigar)) {
	  									...
	  									load_buttons('anchor',array('name'=>'View',
					                                      	...
					                                      	'href'=>ADMIN_PATH."issue_and_receipts/karigar_ledgers/create?karigar=".$karigar."&&hook_kdm_purity=".$purity_index.$type));
	  								}
	  							}
	  						?>
  						</td>
							...
							<td class="text-right"><a href=<?=$href.'type='.str_replace(' ', '_', $type_index).'&&column=melting_wastage&&purity='.$purity_index.'&&karigar='.str_replace(' ', '_', $karigar)?> ><?=isset($balance)?four_decimal($balance):0?></a></td>
							<td class="text-right"><?=isset($balance_fine)?four_decimal($balance_fine):0?></td>
							<td class="text-right"><?=isset($type_data['box_weight'])?four_decimal($type_data['box_weight']+$balance):four_decimal(0+$balance) ?></td>

							<td class="text-right"><?=isset($type_data['in_weight'])?four_decimal($type_data['in_weight']):0 ?></a></td>
							<td class="text-right"><?=isset($type_data['out_weight'])?four_decimal($type_data['out_weight']):0 ?></td>
							<td class="text-right"><?=isset($type_data['melting_wastage'])?four_decimal($type_data['melting_wastage']):0 ?></td>
							<td class="text-right"><?=isset($type_data['daily_drawer_wastage'])?four_decimal($type_data['daily_drawer_wastage']):0 ?></td>
							<td class="text-right"><?=isset($type_data['loss'])?four_decimal($type_data['loss']):0 ?></td>
...
<tr class="bg_cyan bold">
	...
	<td class="text-right bold"><?=four_decimal($sum_process_in_weight);?></td>
	<td class="text-right bold"><?=four_decimal($sum_process_out_weight);?></td>
	<td class="text-right bold"><?=four_decimal($sum_melting_wastage);?></td>
	<td class="text-right bold"><?=four_decimal($sum_daily_drawer_wastage);?></td>
	<td class="text-right bold"><?=four_decimal($sum_loss);?></td>
