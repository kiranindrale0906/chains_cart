...
class Core_forgot_password extends BaseController {
  ...
  public function store(){
    $this->data['redirect_url']=ADMIN_PATH."users/login/create";
    parent::update(0);
  }

  public function _before_save($formdata, $action){
    $user = $this->User_model->get('id',array("email" => $formdata['forgot_password']['email']));
    if(!empty($user)):
      $formdata['forgot_password']['id'] = $user[0]['id'];
      $string = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'.$formdata['forgot_password']['email'];
