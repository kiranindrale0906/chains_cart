...
function coreusers_list_settings() {
  return array(
    ...
    array("User Contact No.", "mobile_no", TRUE, "mobile_no", TRUE, TRUE, 'users.mobile_no as contact_no', 'masters', FALSE,'autocomplete',array('users','mobile_no')),
    ...
    array("Created at", "created_at", TRUE, "users.created_at", TRUE, TRUE, "DATE_FORMAT(users.created_at, '%d-%m-%Y') as created", 'masters', FALSE,'date',array('users','created_at')),
...
function coreusers_get_field_attribute($table, $field) {
  ...
  $attributes['users'] = array(
    ...
    'user_type'  => array('User Type ', 'Select User Type', TRUE, '', TRUE),
