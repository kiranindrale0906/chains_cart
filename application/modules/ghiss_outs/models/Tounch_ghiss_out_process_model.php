<?php 
class Tounch_ghiss_out_process_model extends BaseModel{
	public $router_class = 'tounch_ghiss_out_processes';
    ...
    $rules= array(array('field' => 'tounch_ghiss_out_processes[in_weight]', 'label' => 'Weight',
  ...
  public function before_validate(){
    if (isset($this->formdata['process_out_wastage_details']) 
      ...
      $process = $this->process_model->find('sum(balance_tounch_ghiss) as in_weight,
                                             sum(out_purity * tounch_ghiss) / sum(tounch_ghiss) as out_purity,
                                             sum(out_lot_purity * tounch_ghiss * out_purity) / sum(tounch_ghiss * out_purity) as out_lot_purity',
  ...
  public function save($after_save = true) {
    $srno = $this->process_model->find('count(*) + 1 as srno',
                                       array('product_name'=>'Ghiss Out','department_name'=>'Start'))['srno'];
    
    $start_process=array(
      'department_name' => 'Ghiss Melting',
      'lot_no'=>strtoupper('TGOP-'.sprintf("%02d", $srno)),
    ...
    if (isset($this->formdata['process_out_wastage_details'])) {
      foreach ($this->formdata['process_out_wastage_details'] as $index => $process_out_wastage_detail) {
        if (isset($process_out_wastage_detail['process_id']) && !empty($process_out_wastage_detail['process_id'])) {
          ...
          $process_out_wastage_detail_obj->attributes['out_weight'] = $process['balance_tounch_ghiss'];
          $process_out_wastage_detail_obj->attributes['field_name'] = 'Tounch Ghiss Out';
