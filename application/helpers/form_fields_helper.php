...
function get_field_data($data, $router, $record) {
  ...
  //$data['autocomplete'] = ((isset($data['autocomplete']) && $data['autocomplete'] == true )) ? '' : 'autocomplete="off"';
  ...
  $data['class'] = (isset($field_details[3]) && $field_details[3] != '') ? $field_details[3]: ''. (isset($data['class'])?$data['class']:'');
...
function get_records_by_id($records, $id_field='id') {
  $result = array();
...
function get_records_array_by_id($records, $id_field='id') {
  $result = array();
  foreach ($records as $index => $record) {
    if (!isset($result[$record[$id_field]])) $result[$record[$id_field]] = array();
    $result[$record[$id_field]][] = $record;
  }
  return $result;
}
