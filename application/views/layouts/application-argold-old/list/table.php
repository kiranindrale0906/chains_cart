...
<?php } ?>
 -->
  <table class="table table-sm theme_default fixedTable_js table-bordered" id="customTableId">
    <thead class="thead-dark" id="myHeader">
      ...
      <?php if ($filter_columns != '' && $table_data != '') : ?>
        <tbody>
          <?php if ( ! empty($table_data) && $table_data != '') : ?>
            <?php $row_color_field_value=$tr_background_color=''; ?>
            <?php foreach ($table_data as $index => $value): ?>
              <?php 
                ...
                if ($row_color_field_value != @$value[$page_details['row_color_field']]) {
                  $tr_background_color = ($tr_background_color=='bg_gray') ? '' : 'bg_gray';
                  $row_color_field_value = $value[$page_details['row_color_field']];
                }
              ?>
              <tr class="<?= $tr_background_color ?>">
                <?php foreach ($tablehead as $key => $colum) { ?>
                  <?php if ($key == 'action') { ?>
                    <td class="text-right">                    
                      <?= getActions($value, $table_name, $url, $select_url, $filter_details); ?>
                    </td>
                  <?php } else if ($key == 'checkbox') { ?>
                    <td>                    
                      <input type="checkbox" name="checkbox_ids[]" value="<?= $value['id'] ?>"></input>
                    </td>  
                  <?php } elseif ($key == 'gross_weight') {
                      if (!empty($value[$key]) && $value[$key]!=0) {
                          $gross_weight_txt='<a href="'.ADMIN_PATH.'melting_lots/melting_lots/view/'.$value['id'].'">'.$value[$key].'</a>';
                      }else{
                        $gross_weight_txt='<a href="'.ADMIN_PATH.'melting_lots/sub_melting_lot_details/create/'.$value['id'].'"><i class="fa fa-plus pull-right"></i></a>';
                      }

                   ?>
                    <td class='text-right'><?=$gross_weight_txt?></td>
                  <?php } elseif (in_array($key, $check_image)) { ?>

                    <td><img src="<?= get_image_url($key, $value[$key]) ?>" height="100" width="100"></td>
                  <?php } else { ?>
                    <td>
                      <span><?php echo getColumnData($value[$key], $key, $value['id']); ?></span>
                    </td>
                  <?php } ?>
