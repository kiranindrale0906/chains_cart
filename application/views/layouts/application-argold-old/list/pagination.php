<p class="font12 medium m-0"><?php echo 'Showing '.$pagination['showing'].' to '.$pagination['end'].' of '.$pagination['count'].' entries';?></p>	
		...
		<ul class="pagination pagination_blue pagination-sm m-0">
			<?php if($pagination['page_no']!=1){?>
				<li class="page-item previous" aria-controls="datatable" tabindex="0" id="datatable_previous">
					<a class="page-link" href="<?php echo $pagination['current_url'];?>&page_no=<?php echo $pagination['prev_page_id']?>">Previous</a>
				</li>
			<?php }?>

			<?php if($pagination['pages']>1){
				for($i = $pagination['page_no']; $i <= min($pagination['page_no'] + 9, $pagination['pages']); $i++){
					($i==$pagination['page_no']) ? $class = 'active' : $class = '';?>
						<li class="page-item <?php echo $class; ?>" aria-controls="datatable" tabindex="0">
							<a class="page-link"  href="<?php echo $pagination['current_url']?>&page_no=<?=$i;?>">
								<?=$i;?>
							</a>
						</li>
			<?php }}?>
			...
			<?php if($pagination['pages']!=$pagination['page_no'] && $pagination['pages']!=0){?>
				<li class="page-item next" aria-controls="datatable" tabindex="0" id="datatable_next">
					<a class="page-link" href="<?php echo $pagination['current_url']?>&page_no=<?php echo $pagination['next_page_id']?>">Next</a>
				</li>
			<?php }?>
		</ul>
	</div>
