...
Class Upload_file {
  ...
  private function updateMedia($image,$folder,$fileType,$controller,$field_name,$index='',$is_old_resize=false,$image_data='',$hieght=50,$width=50,$path=FALSE) {
  ...
  private function resize_images($folder, $image_data,$img_sizes_arr){
      ...
      foreach($img_sizes_arr as $k => $v){
        ...
        $resize['quality']            = '100%';
        if(!empty($image_data["image_width"]))
  ...
  public function upload_file_data($controller, $field_name, $index='', $is_new_size=false){
    ...
    $folder = isset($file_content['folder'])?'uploads/'.$file_content['folder']:'uploads/original';
    $upload = '';
  ...
  private function upload_file_settings($controller_name, $field_name,$index=''){
    if(isset($_FILES[$controller_name]['name'][$field_name]) && is_array($_FILES[$controller_name]['name'][$field_name])) {
    ...
    if(!empty($file_name['error']) && !is_api_request())
      $this->ci->session->set_flashdata('error',$file_name['error']);
    if(empty($file_name[0]['error']))  
      return $file_name;
  }
